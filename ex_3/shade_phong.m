function Y = shade_phong(verts_p, verts_n, verts_c, bcoords, cam_pos, mat, lights, Ia, X)

    % Initialize colors
    vertex_colors = zeros(3,3);
    
    vertex_colors = verts_c';
    ...
    for y = ymin:1:ymax    
        ...
        N1 = verts_n(:,idx(1));
        N2 = verts_n(:,idx(2));
        normal_A = vector_interp(p1,p2,a,N1',N2',2);        
        ...
        N1 = verts_n(:,idx(1));
        N2 = verts_n(:,idx(2));
        normal_B = vector_interp(p1,p2,b,N1',N2',2);
                       
        for x = points(1):points(2)
            fill = [x y];            
            temp_color = vector_interp(a, b, fill, A, B, 1);
            
            % Normal vector
            temp_normal = vector_interp(a, b, fill, normal_A, normal_B, 1);
            
            % Add ambient light
            Iamb_temp = ambient_light(mat,temp_color',Ia);
        
            % Add diffuse light
            Id_temp = diffuse_light(bcoords, temp_normal', temp_color', mat, lights);
        
            % Add specular light
            Is_temp = specular_light(bcoords, temp_normal', temp_color', cam_pos, mat, lights);
            
            X(x, y, :) = temp_color + Iamb_temp' + Id_temp' + Is_temp';
        end
